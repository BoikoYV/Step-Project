...
window.addEventListener('DOMContentLoaded', function () {
   ...
   // Переключение табов на экране "Наши услуги"
   toggleServiceTabs();

   // Переключение комментариев на экране "Комментарии"
   changeComments();

   // Добавление и фильтрация карточек на экране "Наши работы"
   filterCards();

   // Формирование галереи плагином Masonry и добавление новых фото
   addPhotos();

})

// -----------Табы на экране наши услуги----------
function toggleServiceTabs() {
...
function changeComments() {
   let step = 0;       // смещение слайда по оси Х
   let slideNum = 1;   //Номер большого активного слайда
...
function filterCards() {
   ...
   // Фильтрация карточек при переключении табов 
   ...
   // Создание массива из 36 карточек с информацией
   ...
   // Массив со всеми карточками( 36шт.)
   let cards = createCardsList();

   // оборачивание сгенерированной карточки в HTML верстку
   function createCard(obj) {
      contentItemsList.innerHTML += `<li class="works__content-item content__item" data-content="${obj.category.split(' ').join('-')}">
      <img class="content__item-img" src="${obj.imgSrc}" width="285" height="206" alt="${obj.imgAlt}">
      <div class="content__item-text content-block">
         <p class="content-block__links">
            <a href="#!" class="content__item-link content-block__source ">
               <svg class="content-block__source-img" width="15" height="15" viewBox="0 0 15 15" fill="#1FDAB5">
   ...
   // Догрузка карточек по кнопке
   let loadBtn = document.querySelector('.works__btn-load');

   loadBtn.addEventListener('click', function () {
      // Добавление/удаление анимации загрузки
...
// ------Офрление галереи плагином Masonry и добавление фотографий------
function addPhotos() {
   let bigGallery = document.querySelector('.gallery__block');
   let count = 0; //Количество добавлений фото в галерею

   // Выстраивание карточек плагином Masonry
   startMasonry();
   ...
   galleryLoadBtn.addEventListener('click', function (e) {
      ...
      // Добавление фото после анимации
      setTimeout(() => {

         // Добавление фото
         addPhotosToGallery();
         // Выстраивание фото плагином Masonry
         startMasonry();

         // Удаление кнопки догрузки
         if (count === 2) {
            this.remove();
         }
      }, 2000)
   ...
   function addPhotosToGallery() {
      ...
      // Добавление имен карточкам
      galleryImgs.length < 13 ? addPhoto(0, 5) : addPhoto(6, 10);

      // Вывод в документ набора карточек
   ...
   // Оборачивание фотографии в HTML для вставки в документ
   ...
   // Преобразование галереи плагином Masonry после загрузки всех картинок в галерее
...
// Удаление/добавление активного класса
